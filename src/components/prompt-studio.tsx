      ...
      <section id="workspace" className="workspace-grid" aria-label="Prompt consultant workspace">
        ...
        <section className="panel result-panel">
          ...
          <div className="tab-row" role="tablist" aria-label={t.resultTabsAria}>
            {tabs.map((tab) => <button key={tab.id} className={activeTab === tab.id ? "tab active" : "tab"} type="button" onClick={() => setActiveTab(tab.id)}>{tab.label}</button>)}
          </div>
          ...
          {!isEnhancing && activeTab === "why" && <div className="simple-result-stack"><div className="consultant-grid"><section className="consultant-card score-summary"><h3>{t.qualityScore}</h3><strong className="big-score">{displayScore}/100</strong><p>{displayGrade}</p></section><section className="consultant-card"><h3>{t.whatImproved}</h3><ul>{(asArray(result.improvements).length ? asArray(result.improvements) : asArray(result.evaluation?.suggestions)).map((item) => <li key={item}>{item}</li>)}</ul></section><section className="consultant-card"><h3>{t.qualityGoals}</h3><ul>{(asArray(result.qualityContract?.mustImprove).length ? asArray(result.qualityContract?.mustImprove) : [t.defaultQualityGoal]).map((item) => <li key={item}>{item}</li>)}</ul></section></div><div className="card-list">{(asArray(result.changes).length ? asArray(result.changes) : [{ title: t.promptRewritten, before: input, after: result.enhancedPrompt, reason: t.rewriteReason }]).slice(0, 3).map((change) => <article className="change-card" key={change.title}><h3>{change.title}</h3><div className="diff-grid"><article><span>{t.before}</span><p>{change.before}</p></article><article><span>{t.after}</span><p>{change.after}</p></article></div><p>{change.reason}</p></article>)}</div>{benchmarkResult && <div className="roadmap-panel"><div className="score-duo"><article><span>{t.original}</span><strong>{benchmarkResult.originalScore}</strong></article><article><span>{t.improved}</span><strong>{benchmarkResult.enhancedScore}</strong></article><article><span>{t.winner}</span><strong>{benchmarkResult.winner}</strong></article></div><p>{benchmarkResult.summary}</p></div>}{optimizeResult && <div className="roadmap-panel"><div className="score-duo"><article><span>{t.finalScore}</span><strong>{optimizeResult.finalScore}</strong></article><button className="secondary-action" type="button" onClick={applyOptimizedPrompt}>{t.useOptimized}</button></div><p>{optimizeResult.reasoningSummary}</p></div>}<div className="workflow-actions"><button className="secondary-action" type="button" onClick={() => void runBenchmark()} disabled={isBenchmarking || !result.enhancedPrompt}>{isBenchmarking ? t.comparing : t.compare}</button><button className="secondary-action" type="button" onClick={() => void runOptimize()} disabled={isOptimizing || !input.trim()}>{isOptimizing ? t.optimizing : t.stronger}</button></div></div>}
          {!isEnhancing && activeTab === "alternatives" && <div className="variant-grid">{(asArray(result.variants).length ? asArray(result.variants) : [{ name: t.improved, prompt: result.enhancedPrompt, bestFor: t.defaultVariant }]).map((variant) => <article className="variant-card" key={variant.name}><div><h3>{variant.name}</h3><span>{variant.bestFor}</span></div><pre>{variant.prompt}</pre><button className="secondary-action" type="button" onClick={() => navigator.clipboard.writeText(variant.prompt)}>{t.copyVersion}</button></article>)}</div>}
          {!isEnhancing && activeTab === "safety" && <div className="simple-result-stack"><div className="consultant-grid"><section className="consultant-card"><h3>{t.questions}</h3><ul>{(asArray(result.clarifyingQuestions).length ? asArray(result.clarifyingQuestions) : [t.noExtraQuestions]).map((item) => <li key={item}>{item}</li>)}</ul></section><section className="consultant-card"><h3>{t.checklist}</h3><ul>{(asArray(result.checklist).length ? asArray(result.checklist).map((item) => `${item.passed ? "✓" : "!"} ${item.item}`) : asArray(result.metrics).map((metric) => metric.label)).map((item) => <li key={item}>{item}</li>)}</ul></section><section className="consultant-card"><h3>{t.safetyRules}</h3><ul>{(asArray(guardrailsResult?.guardrails).length ? asArray(guardrailsResult?.guardrails) : [t.safetyFallback]).map((item) => <li key={item}>{item}</li>)}</ul></section></div><div className="test-panel inline-test"><div><p className="section-kicker">{t.trySafely}</p><h3>{t.testWithContext}</h3></div><textarea value={sampleInput} onChange={(event) => setSampleInput(event.target.value)} placeholder={t.samplePlaceholder} /><div className="workflow-actions"><button className="secondary-action" type="button" onClick={() => void runTestPrompt()} disabled={isTesting || !result.enhancedPrompt}>{isTesting ? t.testing : t.testPrompt}</button><button className="secondary-action" type="button" onClick={() => void runGuardrails()} disabled={isGeneratingGuardrails || !result.enhancedPrompt}>{isGeneratingGuardrails ? t.creating : t.createSafety}</button></div>{testResult && <div className="test-result"><pre>{testResult.previewOutput}</pre><div className="consultant-grid"><section className="consultant-card"><h3>{t.strengths}</h3><ul>{asArray(testResult.strengths).map((item) => <li key={item}>{item}</li>)}</ul></section><section className="consultant-card"><h3>{t.possibleIssues}</h3><ul>{asArray(testResult.failureModes).map((item) => <li key={item}>{item}</li>)}</ul></section><section className="consultant-card"><h3>{t.recommendedTweaks}</h3><ul>{asArray(testResult.recommendedTweaks).map((item) => <li key={item}>{item}</li>)}</ul></section></div></div>}</div></div>}
